...
// RX  PKT_REM_RSP (0x23): Starter→Remote  [0x23][status][light][seqEcho][mac0][mac1][mac2]  7 bytes
//   status = motor state only · light = dedicated light-switch field (1=ON, 0=OFF)
//
...
// Most recent command type sent (set in remSendCmd). Lets remHandleRsp give
// light-switch feedback for RELAY commands vs motor feedback for M1 commands.
...
// [0x23][status][light][seqEcho][mac0][mac1][mac2]  = 7 bytes
...
void remHandleRsp(const uint8_t *buf, uint8_t len) {
    ...
    uint8_t light  = buf[2];   // dedicated light-switch field (1=ON, 0=OFF)
    ...
    // 4a. Light-switch RSP — if the last command was a RELAY (light) command, give
    // light-specific feedback (dedicated `light` field, 1=ON) instead of motor feedback.
    // Button enables still track motor state from `status` so motor control stays
    // correct regardless of the light toggle.
    if (remLastSentCmd == REM_CMD_RELAY_ON || remLastSentCmd == REM_CMD_RELAY_OFF) {
        ...
        buttonEn[2] = ENABLED;
        // Light feedback ONLY when the light is ON (per spec). Light-OFF: no chime/LED.
        if (lightOn) {
    ...
    // 5. Light field already consumed above for RELAY feedback.
    (void)light;
}
