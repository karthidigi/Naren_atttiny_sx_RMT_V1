...
void rxFunc() {
  ...
  if (strcmp(encapData, "1N") == 0) {
    ...
    // Auto-stop / alarm — orange LED (fault colour), 2× 100 ms beep
    buttonEn[0] = ENABLED;   // M1_ON
    ...
    // Bypass switch active on starter — cyan LED (bypass colour), 3× short beep
    buttonEn[0] = ENABLED;
    buttonEn[1] = ENABLED;
    buttonEn[2] = ENABLED;
    funcStaLCyan();
    ...
    // Voltage fault active on starter — orange LED (fault colour), 2× long beep
    buttonEn[0] = ENABLED;
    buttonEn[1] = ENABLED;
    buttonEn[2] = ENABLED;
    funcStaLOrange();
    ...
    // Fault — 2× orange blink + 2× 150 ms beeps (alarm pattern)
    buttonEn[0] = ENABLED;
    buttonEn[1] = ENABLED;
    buttonEn[2] = ENABLED;
    funcStaLOrange(); buzBeep(150); funcLedReset(); delay(100);
    funcStaLOrange(); buzBeep(150); funcLedReset();

  } else if (strcmp(encapData, "D2") == 0) {
    // Bypass — 2× cyan blink + short/long beeps (asymmetric warning)
    buttonEn[0] = ENABLED;
    buttonEn[1] = ENABLED;
    buttonEn[2] = ENABLED;
    funcStaLCyan(); buzBeep(80);  funcLedReset(); delay(100);
    funcStaLCyan(); buzBeep(300); funcLedReset();
