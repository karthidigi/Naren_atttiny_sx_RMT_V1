...
void ackReception() {
  if (msgTxd) {
    ...
    // Second half of the same defect: never start a retransmit while DIO1 still has an
    // unserviced interrupt. The packet waiting in the radio may be the very ack this
    // retry is about to talk over. Deferring costs one 5 ms loop pass.
